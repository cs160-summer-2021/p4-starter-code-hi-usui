export const ERRORS_CLEAR = "ERRORS_CLEAR";
export const ERRORS_GET = "ERRORS_GET";
export const PLAYLIST_LOADING = "PLAYLIST_LOADING";
export const PLAYLIST_SET = "PLAYLIST_SET";
export const PLAYLIST_SONG_ADD = "PLAYLIST_SONG_ADD";
export const PLAYLIST_SONG_REMOVE = "PLAYLIST_SONG_REMOVE";
export const PLAYLIST_USER_ADD = "PLAYLIST_USER_ADD";
export const PLAYLIST_USER_REMOVE = "PLAYLIST_USER_REMOVE";
export const SCAN_END = "SCAN_END";
export const SCAN_START = "SCAN_START";
export const USERS_SET_CURRENT = "USERS_SET_CURRENT";
